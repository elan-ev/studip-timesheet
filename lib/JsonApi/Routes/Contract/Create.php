...
class Create extends JsonApiController
{
    ...
    public function __invoke(Request $request, Response $response, $args)
    {
        ...
        $contract = $this->createContract($json);

        return $this->getCreatedResponse($contract);
    }
    ...
    protected function validateResourceDocument($json, $data)
    {
        ...
        if (!self::arrayHas($json, 'data.attributes.employee-id')) {
            return 'Missing `employee-id` member of attributes block.';
        }
     ...
     * @return Contract
     */
    private function createContract(array $json)
    {
        $employeeId = self::arrayGet($json, 'data.attributes.employee-id');
        $instituteId = self::arrayGet($json, 'data.attributes.institute-id');
        $type = self::arrayGet($json, 'data.attributes.type');
        ...
        $contract->institute_id = $instituteId;
        $contract->type = $type;
