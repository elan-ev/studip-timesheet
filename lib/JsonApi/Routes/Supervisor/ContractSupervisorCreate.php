<?php
/**
 * Contract Supervisor Create Route Handler
 *
...
class ContractSupervisorCreate extends JsonApiController
{
    use ValidationTrait;
    ...
    public function __invoke(Request $request, Response $response, $args)
    {
        ...
        $supervisor = $this->createSupervisor($json, $contract);
     ...
     * @return Supervisor
     */
    private function createSupervisor(array $json, Contract $contract)
    {
        $userId = self::arrayGet($json, 'data.attributes.user-id');

        $supervisor = new Supervisor();
        $supervisor->user_id = $userId;
        $supervisor->contract_id = $contract->id;
        $supervisor->store();
