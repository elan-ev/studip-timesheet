...
class Create extends JsonApiController
{
    ...
    public function __invoke(Request $request, Response $response, $args)
    {
        ...
        $sheetId = self::arrayGet($json, 'data.attributes.sheet-id');
        $sheet = Sheet::find($sheetId);
        if (!$sheet) {
            throw new RecordNotFoundException();
        }

        if (!Authority::canCreateRecord($user, $sheet)) {
            throw new AuthorizationFailedException();
        }

        $record = $this->createRecord($json);
    ...
    protected function validateResourceDocument($json, $data)
    {
        ...
        if (!self::arrayHas($json, 'data.attributes.sheet-id')) {
            return 'Missing `sheet-id` member of attributes block.';
        }
    ...
    /**
     * Extract data and creates record.
     * @param array $json
     * @return Record
     */
    private function createRecord(array $json)
    {
        $sheetId = self::arrayGet($json, 'data.attributes.sheet-id');
