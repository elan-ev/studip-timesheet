<?php
/**
 * Sheet Workflow Log Index Route Handler
 *
...
class SheetWorkflowLogIndex extends JsonApiController
{
    /**
     * @inheritDoc
     */
    ...
    public function __invoke(Request $request, Response $response, $args)
    {
        ...
        if (!Authority::canIndexSheetWorkflowLogs($user, $sheet)) {
            throw new AuthorizationFailedException();
        }

        [$offset, $limit] = $this->getOffsetAndLimit();

        $total = $sheet->workflow_logs->count();
        $data = $sheet->workflow_logs->limit($offset, $limit);
